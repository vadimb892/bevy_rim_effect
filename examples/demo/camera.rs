...
use bevy::{ core_pipeline::{tonemapping::Tonemapping, Skybox}, input::mouse::{MouseMotion, MouseWheel}, prelude::*, render::view::ColorGrading };
...
) 
{
  let transform = Transform::from_translation( 
    Vec3::new( CAMERA_DISTANCE, 0.0, 0.0 ) + FOCUS
  );  
  commands.spawn((
    Camera3dBundle {
      transform,
    ...
    /*Skybox{
      image : texture_assets.skybox.clone( ),
      brightness : 1000.0
    },*/
...
impl UpHemisphereTransform
{
  ...
  fn get( &self ) -> Vec3
  {
    let t = sphere_to_decart( 
      Vec3::new( self.radius, self.pitch, self.yaw ) 
    );
    self.focus + t
  }
...
fn sphere_to_decart( sphere_coords : Vec3 ) -> Vec3 {
  let Vec3{ x :  r, y : mut theta, z : mut phi } = sphere_coords;
